...
export const uploadPowerSyncData = createServerFn({ method: 'POST' })
  .inputValidator((input: { operations: Array<z.infer<typeof operationSchema>> }) => input)
  .handler(async ({ data }) => {
    const parsed = z.object({ operations: z.array(operationSchema) }).safeParse(data)
    if (!parsed.success) {
      return {
        success: false,
...
async function updateRecord(
  ...
  const values = columns.map((column) => opData[column])
  const assignments = columns.map((column, index) => `${quoteIdentifier(column)} = $${index + 2}`).join(', ')

  await client.query(`UPDATE ${quoteIdentifier(table)} SET ${assignments} WHERE id = $1`, [id, ...values])
}
